...
Check(new VibeCatchEditor.App.Editor.EditorView().RendererStatusKey == "ui.renderStatus", "Shared editor preserves the Windows renderer label by default");
...
audio.Pause();
await audio.LoadAsync(Path.Combine(root, "tests", "VibeCatchEditor.Audio.Tests", "Fixtures", "quiet-tone.ogg"));
Check(audio.State.CanPlay && audio.State.DurationMs > 0, "OGG fixture decodes and opens on the native player");
var oldLoad = audio.LoadAsync(Path.Combine(root, "tests", "VibeCatchEditor.Audio.Tests", "Fixtures", "quiet-tone.ogg"));
