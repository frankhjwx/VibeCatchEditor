...
- 在选择工具下点击 slider 的任意 Fruit / Droplet / TinyDroplet，按皮肤实际尺寸命中并选中整条。导入对象点击“编辑 Slider”后转为本地时间—X 轨迹，可改节点、柄与行程次数；转换为一步撤销。
...
- 未编辑的导入 slider 保留 L/B/P/C 原表示。转为可编辑轨迹后保留父 ID、源顺序、repeat 与样本信息；首个 span 的节点由后续行程共享，导入轨迹默认保留原 Tiny RNG。
