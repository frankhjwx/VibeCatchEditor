...
- Legacy Slider 选中后，从右侧属性或右键菜单执行 **转换为 VCE Slider**，再选择锚点拖动或输入数值。一次撤销可恢复 Legacy 表示；无法严格贴合 TinyDroplet 时不替换。
...
| 项目 | 数量 | 本轮重点 |
| --- | ---: | --- |
| Core.Tests `--fixtures` | 42 | 混合段、保形分割、repeat、Legacy 转 VCE、原子失败及两张实图 |
...
Legacy Slider 转换后以折线近似原路径投影，不能恢复原作者的贝塞尔控制柄；转换后可逐段改成贝塞尔。Legacy Slider 保留实际 Tiny RNG 偏移，VCE Slider 尝试让实际对象贴合目标曲线。重复行程共享路径且无法满足贴合时，转换保持 Legacy 表示并提示减少行程次数。仍不加载视频或 storyboard，不加入完整游戏客户端。
