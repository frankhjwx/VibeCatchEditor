...
编辑 slider 需要改变 SV 时，替换开始时刻的旧绿点，避免同刻叠加不同速度；保留红点和有效样本字段。只有下一原 timing 点或下一编辑 slider 之前的未编辑 slider 仍依赖旧速度时才写恢复点，无依赖则省略。恢复使用头部之后的安全整数时间；临近对象无法同时保持速度时明确拒绝。输出时长另用原始字段公式验证，不能只以自身 reader 回读成功作为依据。
