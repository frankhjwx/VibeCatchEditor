...
- Fruit 工具支持 1/4、1/6 拍；贝塞尔工具向上添加锚点，Enter 完成，选择后拖动锚点及控制柄。
- slider 生成 Fruit、Droplet、TinyDroplet；tick 对齐目标曲线。Tiny 贴合开关控制 RNG 补偿，无解时显示诊断。Tick ×n 与编辑吸附独立。
...
转换目前限单 timing、单程 slider 和当前模型的完整对象上下文。repeat、香蕉雨、多 timing、mods、文件整数坐标量化/回读及固定 stable 客户端逐帧对照未完成。RNG 补偿可失败，不能宣称任意曲线都能无误差输出。
