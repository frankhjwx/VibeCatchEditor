...
- **控制点与方向柄**：整条 VCE Slider 已选中时，单击锚点直接进入锚点编辑；当前锚点使用独立高对比色并可直接拖动。只有右键实际命中的锚点显示曲线 / 直线转换；时间递增及 X=0..512 约束始终生效，包括尚未连接下一段的草稿末端出柄。
- **右键轨迹 → 插入控制点**：按点击位置插入默认无柄点；右键控制点或属性按钮可转换为曲线 / 直线控制点。邻点仍有柄时，相邻段可以继续弯曲，默认插点不保证保形；需要保持形状时使用“分割插点 · 保持形状”。
- **Legacy 与 repeat**：Legacy Slider 的属性和右键菜单提供“转换为 VCE Slider”。转换要求 TinyDroplet 严格贴合；共享 repeat 路径发生 RNG 冲突时保留 Legacy Slider 并显示原因。
