...
internal static class MacPaths
{
    ...
    private static string FindArtifacts()
    {
        ...
        {
            if (File.Exists(Path.Combine(directory.FullName, "VibeCatchEditor.sln"))) return Path.Combine(directory.FullName, "artifacts");
            directory = directory.Parent;
        }
        return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "Library", "Application Support", "VibeCatchEditor");
    }
